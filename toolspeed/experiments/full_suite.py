"""Full Suite Benchmark Runner across E1-E5 and Workloads W1-W7."""
...
class SuiteRunner:
    """Orchestrates comprehensive multi-trial benchmarking across E1-E5 and W1-W7."""
    ...
    def run_workload_benchmarks(self) -> Dict[str, WorkloadBenchmarkResult]:
        """Execute focused benchmark scenarios for each workload family W1-W7."""
        ...
        w1_red = (w1_s.baseline_p95_ms - w1_s.candidate_p95_ms) / w1_s.baseline_p95_ms * 100.0
        ...
        w2_red = (w2_s.baseline_p95_ms - w2_s.candidate_p95_ms) / w2_s.baseline_p95_ms * 100.0
        ...
        # W3: Branching workflows (Confidence-gated speculation on branch choice)
        ...
        w3_red = (w3_s.baseline_p95_ms - w3_s.candidate_p95_ms) / w3_s.baseline_p95_ms * 100.0
        ...
        # W4: Repeated workflows with high plan locality (JIT Cache + Exact AST execution)
        ...
        # Plan cache hit replaces all intermediate LLM decisions with 8ms cache lookup
        cache_hit_latency = 8.0
        w4_cand = cache_hit_latency + tools4.sum(axis=1) + fin4
        w4_s = compute_summary(w4_base, w4_cand)
        w4_red = (w4_s.baseline_p95_ms - w4_s.candidate_p95_ms) / w4_s.baseline_p95_ms * 100.0
        ...
        # W5: Large tool arguments and results (Action Bytecode + Commit Horizon)
        rng5 = np.random.default_rng(self.seed + 10005)
        gen5 = samples(rng5, 800.0, self.profile.sigma, n)  # large schema decode
        ...
        # Action bytecode reduces decode time by 3.5x, commit horizon starts tool at 40%
        cand_gen5 = (0.4 * gen5) + (0.6 * gen5 / 3.5)
        w5_cand = np.maximum(cand_gen5, 0.4 * gen5 + tool5) + fin5
        w5_s = compute_summary(w5_base, w5_cand)
        w5_red = (w5_s.baseline_p95_ms - w5_s.candidate_p95_ms) / w5_s.baseline_p95_ms * 100.0
        ...
        # W6: Cold-start code/browser sandboxes (Prewarming pool)
        ...
        w6_red = (w6_s.baseline_p95_ms - w6_s.candidate_p95_ms) / w6_s.baseline_p95_ms * 100.0
        ...
        # W7: Side-effecting actions requiring approval and idempotency
        ...
        # Commit horizon on confirmed action with zero duplicates
        w7_base = dec7 + tool7 + fin7
        w7_cand = np.maximum(dec7, 0.45 * dec7 + tool7) + fin7
        w7_s = compute_summary(w7_base, w7_cand)
        w7_red = (w7_s.baseline_p95_ms - w7_s.candidate_p95_ms) / w7_s.baseline_p95_ms * 100.0
    ...
    def run(self) -> SuiteResult:
        """Run all experiments E1-E5 and workload benchmarks W1-W7."""
        ...
        # Check central hypothesis across workloads:
        # "The central hypothesis is wrong for a workload when, after equalising success,
        # cost, concurrency, and safety, no tested mechanism improves P95 CCL by at least 10%."
        central_passed = all(w.central_hypothesis_passed for w in workloads.values())

        # Evidence Log Rows matching research plan template:
        # | Experiment | Tested | Succeeded | Failed | Still unproven | Next action |
        evidence_log = [
            {
                "experiment": "Phase 0 Simulator",
                "tested": "Yes",
                "succeeded": "Mechanisms behave as expected under declared statistical profiles",
                "failed": "No real-world transport noise injected",
                "still_unproven": "Real remote network jitter and live model token streaming",
                "next_action": "Instrument live synchronous baseline adapters",
            },
...
def run_full_suite(
    ...
    """Convenience runner for full suite."""
