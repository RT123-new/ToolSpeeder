...
def fusion(profile, n, seed):
    ...
    for steps in (2, 4, 8, 16):
        ...
        decisions = samples(rng, profile.model_decision_ms, profile.sigma, (n, steps))
        final = samples(rng, profile.model_final_ms, profile.sigma, n)
        tools = samples(rng, profile.tool_ms, profile.sigma, (n, steps))
        overhead = samples(rng, profile.program_runtime_overhead_ms, profile.sigma / 2, n)
...
def speculation(profile, n, seed):
    rows = []
    for mode_index, mode in enumerate(("no_contention", "cancellable", "single_slot")):
        for accuracy in np.linspace(0.0, 1.0, 21):
            rng = np.random.default_rng(seed + 1000 + mode_index * 100 + int(accuracy * 1000))
            ...
            draft = samples(rng, profile.draft_model_ms, profile.sigma / 2, n)
...
def cache(profile, n, seed):
    ...
    for stale_on_hit in (0.0, 0.001, 0.01):
        for hit_rate in np.linspace(0.0, 1.0, 11):
            rng = np.random.default_rng(seed + 2000 + int(stale_on_hit * 1_000_000) + int(hit_rate * 1000))
            model = samples(rng, profile.model_decision_ms, profile.sigma, n)
            tool = samples(rng, profile.tool_ms, profile.sigma, n)
            hit = rng.random(n) < hit_rate
            stale = hit & (rng.random(n) < stale_on_hit)
            baseline = model + tool
            candidate = model + np.where(hit, profile.cache_lookup_ms, tool)
            row = {
...
def decode_compression():
    ...
    for decode_share in (0.10, 0.25, 0.50, 0.80):
        for factor in (2.0, 4.0, 6.0):
            speedup = 1.0 / ((1.0 - decode_share) + decode_share / factor)
            rows.append(
                {
...
def commit_horizon(profile, n, seed):
    ...
    for fraction in np.linspace(0.1, 1.0, 10):
        ...
        generation = samples(rng, profile.model_decision_ms, profile.sigma, n)
        tool = samples(rng, profile.tool_ms, profile.sigma, n)
        baseline = generation + tool
        candidate = np.maximum(generation, fraction * generation + tool)
...
def main():
    ...
    outputs = {
        ...
        "commit_horizon.csv": commit_horizon(profile, args.trials, args.seed),
    }
    ...
    plt.figure(figsize=(8, 5))
    plt.plot(par["independent_calls"], par["p50_speedup"], marker="o", label="P50")
    plt.plot(par["independent_calls"], par["p95_speedup"], marker="o", label="P95")
        ...
        json.dumps(
            {
                ...
                "seed": args.seed,
                "warning": (
                    "Synthetic mechanism sanity-check only. Replace sampled latencies with real instrumented adapters."
                ),
            },
            indent=2,
        ),
